...
>
> 在第 11 章中将会看到, 陷入帧的功能是 **服务当前进程与内核之间的切换**. 具体来讲, 从内核态返回任何进程之前, 都必须先从该进程的陷入帧中恢复所有通用寄存器的值 (通过调用 `userret()` 函数, 见 `kernel/trampoline.S[100:151]`). 反过来, 任何进程陷入内核的第一步都是将所有通用寄存器保存到它自己的陷入帧中 (通过调用 `uservec()` 函数, 见 `kernel/trampoline.S[20:98]`).
