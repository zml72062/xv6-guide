...
* 当 `mtvec` 的低 2 位为 `01` 时, 代表在 M 模式下, 发生中断 i 时, 硬件就跳转到以 `mtvec - 1` (4 字节对齐) 为基址, 偏移量 `4 * mcause` 字节的地址去执行.
> 当一个中断发生时, `mcause` 会按照确定的编码存储这个中断的类型 (也就反映了中断的起因). 具体来说, `mcause` 的最高位存储了这是一个 interrupt (1) 还是 exception (0), 而剩余位则存储了中断码. (见第 2 章的表格 "RISC-V 支持的中断")
