...
有时, 我们希望 CPU 不要被外中断打断. 通过 **中断开关** 可以实现中断机制的开启和关闭. 有两种中断开关:
...
> RISC-V 规定: **对于特权级低于当前特权级的中断, 总是关闭; 对于特权级高于当前特权级的中断, 总是开启**. 这一规定不受到 `xIE` 比特的影响. 因此, 假如 S 模式或 U 模式下发生了一次特权级为 M 的中断, RISC-V 总会陷入到 M 模式, 即便 `MIE=0`.
* **部分中断开关.** `mie` 寄存器包含一系列比特位, 每一个比特位对应一种外中断. 假如将外中断 i 对应的 `mie` 比特位置 0, 那么就禁止响应外中断 i. 
> 类似地, 在 S 模式可以访问 `sie` 寄存器, 它是 `mie` 的子集.

****

总结一下发生陷入和从陷入返回时, `mstatus` 的变化:
...
如上所述, RISC-V 默认要求所有的中断都必须陷入到 M 模式处理. 但是, 操作系统通常运行在 S 模式下. 这意味着操作系统不能直接处理中断. 为此, RISC-V 提供了 **中断代理** 机制. 

RISC-V 提供了 `mideleg` 和 `medeleg` 寄存器, 分别代表 interrupt delegation 和 exception delegation. 如果将 `mideleg`/`medeleg` 寄存器的第 i 位设为 1, 那么就说明将序号为 i 的外中断/内中断 **代理给 S 模式处理**. 各种中断对应的序号如下表所示:
...
```
这段代码做的事情是:
* 将 `mideleg` 和 `medeleg` 的低 16 位设为 1. 也就是说, 我们 **把所有内中断/外中断全部代理给 S 模式处理**.
* 将 `sie` 中 SEIE (对应上表中 supervisor external interrupt), STIE (对应 supervisor timer interrupt), SSIE (对应 supervisor software interrupt) 这三个比特设为 1. 这样, **S 模式的所有外中断都被打开**. (当然, 要在 S 模式真正启用中断, 还需把 `sstatus` 的 SIE 位设为 1.)


## 从 M 模式到 S 模式

现在我们来解释 `start()` 函数中的剩余部分:
...
```c
  ...
  w_mepc((uint64)main);
```
这行代码把 `mepc` 设置为 `main()` 函数 (定义在 `kernel/main.c` 中) 的地址. 寄存器 `xepc` (`x=m/s/u`) 表示: 当我们从一次到特权级别 x 的陷入返回时 (通过 `xret` 指令), 应该把 PC 设置成什么值? 
...
```c
  ...
  w_satp(0);
```
`satp` 是一个在 S 模式管理地址翻译和保护 (address translation and protection) 的寄存器. 当它的最高位 (对于 RV32) 或最高 4 位 (对于 RV64) 全置 0 时, 代表禁用地址翻译, 即机器指令中所有的地址就是物理地址.
...
```c
  ...
  w_pmpcfg0(0xf);
```
这两条指令把物理地址 $[0,2^{56}-1]$ 的范围设为所有模式 (M/S/U) 都可读、可写、可执行的. 

```c
  // ask for clock interrupts.
  timerinit();
```
`timerinit()` 函数完成了对时钟中断的初始化. 这个函数定义在 `kernel/start.c[57:89]`.
...
```
执行 `mret` 指令. 这条指令的效果是:
* `mstatus` 中的 MIE 设为 MPIE
* 特权级别被设为 `mstatus` 中 MPP 的值, 也就是 **S 模式**
