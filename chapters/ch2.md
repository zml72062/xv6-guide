...
* **全局中断开关.** 这些开关位于 `mstatus` 寄存器中:
  + `mstatus` 寄存器中的 MIE/SIE/UIE 比特位表示 "(M/S/U) interrupt enabled". 如果当前特权级别是 x, 那么 `xIE=1` 表示当前 **允许特权级为 x 的外中断**, `xIE=0` 表示当前 **不允许特权级为 x 的外中断**.
  + `mstatus` 寄存器中的 MPIE/SPIE/UPIE 表示 "(M/S/U) previously interrupt enabled". 如果当前特权级别是 x, 那么 `xPIE` 用来保存 **陷入到当前特权级别 x 之前 `xIE` 的值**.
> 请注意: 设置 `xIE=0` 只是在 **处理器特权级等于 x 时, 禁用特权级为 x 的外中断**. 假如处理器特权级低于 x, 那么 RISC-V 规定 **总是要响应** 特权级为 x 的中断 (即中断发生时总会 trap 到 x 特权级), 而不受 `xIE` 比特的影响. 反过来, 如果处理器特权级高于 x, 那么 **总是忽略** 特权级为 x 的中断. 因此, 假如 S 模式或 U 模式下发生了一次特权级为 M 的中断, RISC-V 总会陷入到 M 模式, 即便 `MIE=0`.
>
