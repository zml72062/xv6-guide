...
* 如果找到了状态为 `UNUSED` 的进程 `p`, 就对它做初始化工作: (注意此时 `p->lock` 没有被释放, 因此我们可以修改 `p` 的状态字段)
    ...
    + 初始化 `p` 的 **上下文**. 在 xv6 中, 进程的上下文被定义为如下结构体: (`kernel/proc.h[1:19]`)
        ...
        > * `context` 结构体仅用于 **进程调度**. 所谓进程调度, 就是把执行权从一个进程转移给另一个进程. 在第 11 章中我们将会看到, xv6 通过两个函数 `scheduler()` (`kernel/proc.c[437:472]`) 和 `sched()`(`kernel/proc.c[474:499]`) 的相互配合, 来完成进程调度. 其中
        ...
        >
        > 和 `trapframe` 相比, 可以认为 `context` 是相对 "轻量级" 的上下文. 一个很自然的问题是: 为什么对于进程调度而言, 保存和恢复 "轻量级" 的上下文就足够了, 但是对于陷入及陷入返回, 则需要保存和恢复 "完整" 的上下文呢? 
        >
        > 关键的区别在于, 进程调度总是由操作系统 **主动发起** 的, 而陷入是由 **中断** 引起的. 具体来说,
        ...
        > * 与进程调度不同, 陷入是由硬件的中断机制引起的. 中断具有 **不可预测性**: 处理器每执行完一条指令都有可能被中断打断. 由于中断的这一特点, 编译器不可能也无需为将要发生的中断保存 caller-saved 寄存器. 因此, 必须由中断处理函数来保证中断发生前后 **所有通用寄存器** 的完整性.
