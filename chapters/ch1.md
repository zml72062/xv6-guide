...
作为一个简单的例子, 我们来调试一下 QEMU 模拟器启动后执行的最初几行代码. 按照上面所述的方法配置环境并启动 `gdb` 远程调试后, 应该会看到以下输出:
```
0x0000000000001000 in ?? ()
```
这意味着 QEMU 启动后, 会从 **地址 `0x1000`** 开始执行. 

> 如果仔细研究一下我们上面提到过的 `qemu -machine virt` 物理地址空间布局的话, 可能会注意到这样一行注释: (`kernel/memlayout.h[6]`)
> ```c
> // 00001000 -- boot ROM, provided by qemu
> ```
> 也就是说, 从物理地址 `0x1000` 出发的一段内存区域中, 存放了 QEMU 的启动代码. 这段代码不属于 xv6 的一部分, 而是 QEMU 模拟器预先写好的 (就如同 hard-wired 于硬件中).

一个很自然的问题是, **这段启动代码究竟是什么呢?** 有人可能会尝试用 `disas` 命令反汇编这段代码, 但是不幸的是, 由于这段代码不处于任一 **全局函数** 的辖域内, `disas` 命令无法提供它的反汇编:
```
(gdb) disas
No function contains program counter for selected frame.
```
解决方案有两种. 第一种是使用 examine-as-instruction 命令 `x /i` 或 `x /<number>i`:
```
(gdb) x /i 0x1000
=> 0x1000:      auipc   t0,0x0
(gdb) x /7i 0x1000
=> 0x1000:      auipc   t0,0x0
   ...
   0x1018:      unimp
```
`gdb` 命令 `x /i <addr>` 可以反汇编地址 `<addr>` 出发的第一条指令; 进一步地, 命令 `x /<number>i <addr>` 可以反汇编地址 `<addr>` 出发的前 `<number>` 条指令. 

还有一种解决方案是使用 `layout asm` 命令. 该命令会使 `gdb` 窗口变成如下布局:
```
======================================================
  > 0x1000  auipc   t0,0x0
    0x1004  addi    a2,t0,40
...
* 最后的两条指令
  ```asm
      ...
      jr      t0
  ```
  将地址 `t0 + 24` (也就是 `0x1018`) 处的 8 字节整数加载到 `t0`, 然后跳转到该整数所对应的地址接着执行.

为了查看地址 `0x1018` 存储了什么值, 可以执行以下 `gdb` 命令:
```
(gdb) si
...
$2 = 0x80000000
```
结果表明, 地址 `0x1018` 处的 8 字节整数为 `0x80000000`. 因此, 当 QEMU 的启动代码执行完成后, 处理器会跳转到 **物理地址 `0x80000000`** 继续执行. 而 xv6 代码的入口 `_entry` 正好被加载到这个物理地址. 所以从现在开始, 控制就转移到 xv6 的代码中. 

至此, 我们对于 "QEMU 是如何让 xv6 系统开始运转的" 这一问题有了更进一步的认识.
